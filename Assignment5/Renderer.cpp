    ...
    if (auto payload = trace(orig, dir, scene.get_objects()); payload) {
        ...
        case REFLECTION_AND_REFRACTION: {
            ...
            Vector3f reflectionRayOrig =
                (dotProduct(reflectionDirection, N) < 0) ? hitPoint - N * scene.epsilon : hitPoint + N * scene.epsilon;
            Vector3f refractionRayOrig =
                (dotProduct(refractionDirection, N) < 0) ? hitPoint - N * scene.epsilon : hitPoint + N * scene.epsilon;
        ...
        default: {
            ...
            for (auto &light : scene.get_lights()) {
                ...
                specularColor += powf(std::max(0.f, -dotProduct(reflectionDirection, dir)), payload->hit_obj->specularExponent) *
                                 light->intensity;
            }

            hitColor =
                lightAmt * payload->hit_obj->evalDiffuseColor(st) * payload->hit_obj->Kd + specularColor * payload->hit_obj->Ks;
            break;
        }
        }
    }
...
void Renderer::Render(const Scene &scene) {
    ...
    float imageAspectRatio = static_cast<float>(scene.width) / scene.height;
...
#ifdef MSAA2
            Vector3f col[4] = {0, 0, 0, 0};
            int      mm     = 0;
            for (int jj = 0; jj < 2; ++jj) {
                for (int ii = 0; ii < 2; ++ii) {
                    // generate primary ray direction
                    float x = i + 0.25f + ii * 0.5f;
                    float y = j + 0.25f + jj * 0.5f;

                    float xx = (2 * x / scene.width - 1) * imageAspectRatio * scale;
                    float yy = (1.0f - 2 * y / scene.height) * scale;
                    ...
                    Vector3f dir = normalize(Vector3f(xx, yy, -1)); // Don't forget to normalize this direction!
                    col[mm++]    = castRay(eye_pos, dir, scene, 0);
                }
            }
            Vector3f sumCol{0};
    ...
    std::stringstream fienameSS;
#ifdef MSAA2
    fienameSS << scene.width << "_" << scene.height << "_MSAA2.ppm";
#else
    fienameSS << scene.width << "_" << scene.height << ".ppm";
#endif

    FILE *fp;
    int   r = fopen_s(&fp, fienameSS.str().c_str(), "wb");
    if (r) std::cout << "error: " << r << std::endl;
